				...
				.filter(x -> !x.isEmpty()).map(plan -> {
					...
					List<EmployeeCapabilityDTO> employeeCapabilityDTOs = Optional.ofNullable(planDetails)
							.filter(x -> x.getPlanName().equalsIgnoreCase("PLATINUM")).map(v -> {
								...
								List<EmployeeCapabilityDTO> list = info.getEmployeeOfficialInfo()!=null && info.getEmployeeOfficialInfo().getDepartment()!=null && info.getEmployeeOfficialInfo().getDepartment().equalsIgnoreCase(EMPLOYEE_MANAGEMENT)? new ArrayList<>() : collect.get(EMPLOYEE_MANAGEMENT);
